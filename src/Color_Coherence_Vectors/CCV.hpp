...
//	CCV(Texture* t, int numColors, int coherenceThreshold);
	...
	*/
	...
	CCV(const cv::Mat &t, int numColors);
	...
	void calculateImgCCV(const cv::Mat &src);

	/*////////////////////////////////////////////////////////////////////////*/
	//The CCV_hist for the r channel
	std::map< uchar, std::vector<double> > m_CCV_r_hist;

	//The CCV_hist for the g channel
	std::map< uchar, std::vector<double> > m_CCV_g_hist;

	//The CCV_hist for the b channel
	std::map< uchar, std::vector<double> > m_CCV_b_hist;

	void calculateImgCCV_hist(const cv::Mat &src);

	/////////////////////////////////////////////////////////////////////////////
	std::map< uchar, std::vector<double> > m_CCV_q_hist;
	void calculateImgCCV_Qhist(const cv::Mat &src);
...
private:
	...
	*/
	...
	std::map< uchar, std::vector<double> > calculateCCV_hist(cv::Mat img);
	ushort getCoherenceID(ulong area);
	...
	static const double coherenceLevel[10];
};
